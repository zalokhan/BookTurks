...
class QuizAdapterTest(TestCase):
    ...
    def test_create_model(self):
        ...
        self.assertEqual(self.quiz_adapter.get_quiz_for_owner(quiz_name="quiz_name", user=user).count(), 0)
        quiz_model = self.quiz_adapter.create_and_save_model(quiz_name="quiz_name",
                                                             quiz_description="quiz_description",
                                                             quiz_owner=user)
        self.assertEqual(self.quiz_adapter.get_quiz_for_owner(quiz_name="quiz_name", user=user).all()[0], quiz_model)
